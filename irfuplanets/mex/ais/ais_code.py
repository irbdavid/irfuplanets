...
import irfuplanets.mex.ais.morgan as morgan
import irfuplanets.planets.mars.constants as constants
from irfuplanets.data import angle_difference
...
DIGITIZATION_BANNED_KEYS = [
    ...
    "_ion_data",
    "_cyc_data",
    "_morphology_fp_locations",
]
DIGITIZATION_ARRAY_KEYS = [
    "traced_delay",
    "traced_frequency",
    "traced_intensity",
    "morphology_fp_local_selected_f",
...
def ne_to_fp(ne, error=None):
    if error is not None:
        return np.sqrt(ne) * 8980.0, 8980.0 * 0.5 * error / np.sqrt(ne)
...
def td_to_modb(td, error=None):
    if error is not None:
        return (
            2.0 * np.pi / (1.758820150e11 * td),
            2.0 * np.pi / (1.758820150e11 * td**2.0) * error,
        )
...
ais_fp_local_sensible = list(map(ne_to_fp, (1.0, 10000.0)))
...
def _arg_peak(
    ...
    for i in range(width + 1, out.shape[0] - width - 1):
        out[i] = x[i] - 0.5 * (
            np.mean(x[i - width : i]) + np.mean(x[i + 1 : i + width + 1])
        )
        # out[i] = (x[i] - 0.5 * (np.mean(x[i-width:i]) +
        #       np.mean(x[i+1:i+width+1])))
...
def _arg_peak_test(*args, **kwargs):
    plt.close("all")
    plt.figure()
    plt.subplot(211)
    x = np.random.randn(100)
    x = x + (np.arange(100) / 100.0) ** 2.0 * 4.0
    sigma = 4.0
    ...
    plt.subplot(212)
    y, out = _arg_peak(x, full_output=True, *args, **kwargs)
    print(y)

    dx = 0.5 * _fwhm(x, peak_inx=y)
    print("dx = ", dx)
    plt.plot(out)
    plt.plot(y, x[y], "ro")
    plt.vlines(np.array((y[0] - dx, y[0] + dx)), *plt.ylim())
    plt.hlines(3.0, *plt.xlim())
    plt.show()
...
def _proc_peaks(x, max_iter=3, edges=False):
...
def _fwhm(x, y=None, peak_inx=None, *args, **kwargs):
    if y is None:
    ...
    if peak_inx is None:
        peak_inx = _arg_peak(y, *args, **kwargs)

    hm = 0.5 * y[peak_inx] + np.mean(y)
    # inx = np.hstack(
    #     (np.arange(0, peak_inx), np.arange(peak_inx + 1, x.shape[0]))
    # )
...
def laminated_delays(d, f, fp_local, altitude=None):
    ...
    def func(i, j):
        return (
            ...
            / const_c
            * np.log(
                (1.0 - np.sqrt(1.0 - (f[i] / f[j]) ** 2.0))
                / (1.0 + np.sqrt(1.0 - (f[i] / f[j]) ** 2.0))
                * (1.0 + np.sqrt(1.0 - (g[i] / f[j]) ** 2.0))
                / (1.0 - np.sqrt(1.0 - (g[i] / f[j]) ** 2.0))
            )
        )
...
def _remove_none_edge_intersecting(img, edge=0, width=1, as_list=False):
    ...
    def __init__(
        ...
        if remote:
            if remote.lower() == "iowa":
                raise IOError("Iowa MARSIS HTTP no longer exists")
                # self.remote_url = "http://www-pw.physics.uiowa.edu/
                #   plasma-wave/marsx/restricted/DATA/
    ...
    def _get_brain_file(self, time, copy_to_local=None):
        ...
        if copy_to_local:
            ...
            if os.path.exists(fname) and not self.overwrite:
                if self.verbose:
                    print(
                        "Local file already %s already exists, "
...
def read_ais(start, finish=None, input_format=None, verbose=False):
    from irfuplanets.mex.ais import file_manager
...
def read_ais_file(file_name, verbose=False, debug=True):
    ...
    if ionogram_list[-1].time == tmp_ionogram.time:
        # TODO: An example of single freqs: "FRM_AIS_RDR_22106.DAT"
        raise IOError(
...
class Ionogram(object):
    ...
    def find_position(self):
        ...
        pos, mso_pos, sza = mex.mso_r_lat_lon_position(
            float(self.time), sza=True, mso=True
        )  # all in degrees
    ...
    def check_frequency_table(self, validate=False):
        # print self.time < utcstr_to_spiceet("2005-03-14")
        if self.time is not None:
            if self.time < utcstr_to_spiceet("2005-03-14"):
                return False
            return True
        else:
            return False
    ...
    def plot(
        ...
        vmax=ais_vmax,
        color="blue",
        overplot_digitization=False,
        title=None,
        errors=True,
        show_thresholded_data=False,
        no_title=False,
        overplot_model=False,
        altitude_range=None,
        verbose=False,
        labels=True,
        overplot_expected_ne_max=False,
        alpha=1.0,
    ):
        ...
        if not self.linear_frequency:
            if verbose:
                print("Ionogram.plot: Interpolating")
        ...
        if show_thresholded_data:
            if not hasattr(self, "_cyc_data"):
                if verbose:
                    print("Ionogram.plot: Generating binary arrays")
                self.generate_binary_arrays()
            data = self._cyc_data * (vmax - vmin) + vmin

        if verbose:
            print("Ionogram.plot: Imshow")
        img = plt.imshow(
            data,
            vmin=vmin,
            vmax=vmax,
        ...
        if colorbar:
            if verbose:
                print("Ionogram.plot: Colorbar")
            cbar = plt.colorbar(
                img,
                ticks=[-16, -15, -14, -13, -12, -11, -10],
                cax=make_colorbar_cax(ax=ax),
            )
            cbar.set_label(r"Log($V^2m^{-2}Hz^{-1}$)")

        tit = "Ionogram: " + utcstr(float(self.time), format="C")[:-4]
        ...
        if overplot_model:
            if verbose:
                print("Ionogram.plot: Model (2)")

            do_compute = True
            if self._model_sza is not None:
                if abs(self.sza - self._model_sza) < 0.1:
                    do_compute = False

            if do_compute:
                self._model_sza = self.sza
                if verbose:
                    print("Ionogram.plot: Model 1")
                if self.sza < 90.0:
                    if verbose:
                        print(
                            "Ionogram.plot: Model 1.5",
                            self.mex_altitude,
                            np.deg2rad(self.sza),
                        )
                    ...
                    (
                        self._model_delays,
                        self._model_frequencies,
                    ) = Morgan2008ChapmanLayer().ais_response(
                ...
                if verbose:
                    print("Ionogram.plot: Model 2")
                plt.plot(
                    self._model_frequencies / 1e6,
                    -1.0 * self._model_delays * 1e3,
                    color="white",
                    lw=2.0,
                    alpha=alpha,
                )
        ...
        if overplot_expected_ne_max:
            ...
            plt.plot(
                ne_to_fp(ne_max) / 1e6,
                2.0 * (self.mex_altitude - h_max) / speed_of_light_kms * 1.0e3,
                "wo",
                zorder=1000,
            )
        ...
        if self.digitization and overplot_digitization:
            ...
            x0, x1 = plt.xlim()
            plt.hlines(
                2.0 * self.mex_altitude / speed_of_light_kms * 1.0e3,
                3,
                x1,
                colors=color,
                linestyle="dotted",
            ...
            if np.isfinite(d.fp_local):
                ...
                tit += ", Ne0 = %.1f +/- %.1f / cc" % fp_to_ne(
                    d.fp_local, d.fp_local_error
                )
            ...
            if d.traced_delay.size > 0:
                plt.plot(
                    d.traced_frequency / 1.0e6,
                    d.traced_delay * 1.0e3,
                    color=color,
                    marker="+",
                    alpha=alpha,
                )
            ...
            if np.isfinite(d.td_cyclotron):
                ...
                td = np.arange(1.0, 5.0) * d.td_cyclotron * 1e3
                if td.shape[0] > 1:
                    plt.hlines(
                        td,
                        xmin,
        ...
        if altitude_range:
            ...
            ax.set_ylim(
                bottom=2.0
                / speed_of_light_kms
                * (self.mex_altitude - altitude_range[0])
                * 1e3
            )
            ax.set_ylim(
                top=2.0
                / speed_of_light_kms
                * (self.mex_altitude - altitude_range[1])
                * 1e3
            )
    ...
    def interpolate_frequencies(self):
        """Interpolate such that frequencies are at fixed spacing.
        No information lost, but the data volume goes up by ****"""
        ...
        new_frequencies = np.arange(0.0, np.amax(self.frequencies), freq_sep)
    ...
    def threshold_data(self, threshold=10.0**-15):
    ...
    def generate_binary_arrays(self, *args, **kwargs):
        ...
        # of 'one column'. Highest taken to be first line, then take remaining
        # as harmonics if they are above 50% of first
        ...
        for v in np.unique(s):
            ...
            line = [
                ...
                np.mean(tmp_nz[0]),
                np.sum(tmp),
                np.max(tmp_nz[0]) - np.min(tmp_nz[0]),
            ]
            if line[2] > 80:
                line[2] = 80
            # print 'line = ', line
        ...
        if self._morphology_fp_locations:
            ...
            self._morphology_fp_locations = sorted(
                self._morphology_fp_locations, key=lambda x: -x[2]
            )
            threshold = int(self._morphology_fp_locations[0][2] * 0.3)
            # print 'threshold',threshold
            # only those greater than 50% of the largest retained
            self._morphology_fp_locations = [
                v for v in self._morphology_fp_locations if v[2] >= threshold
            ]
        ...
        if self._morphology_fp_locations:
            ...
            self._morphology_fp_locations = sorted(
                self._morphology_fp_locations, key=lambda x: x[0]
            )
        ...
        self.quality_factor = 1.0 - np.sum(
            np.logical_and(
                self.thresholded_data,
    ...
    def calculate_fp_local(self, figure_number=False):
        """Take the first processed line, calculate an error based on
        the remaining.

        Requires that interpolate_frequencies() has been called to make
        the frequency spacing linear.
        ...
        if not hasattr(self, "_morphology_fp_locations"):
            return (
                "Auto-fp_local: Failed (No plasma lines detected in "
        ...
        if figure_number:
            ...
            plt.ylim(0.0, 1.1 * np.amax(scaled))
            plt.vlines(det_fp_local * 1e-6, *plt.ylim(), color="blue")
            plt.vlines(
                2.0 * det_fp_local * 1e-6,
                *plt.ylim(),
                color="blue",
                linestyle="dashed",
            )
        ...
        if (det_fp_local < ais_fp_local_sensible[0]) | (
            ...
            msg = "Auto-fp_local: Failed (result %f not sensible)" % fp_to_ne(
                det_fp_local
            )
            return msg
        ...
        if figure_number:
            if np.isfinite(self.digitization.fp_local):
                plt.vlines(
                    self.digitization.fp_local * np.arange(100) * 1e-6,
                    *plt.ylim(),
                    linestyles="dashed",
                )
            plt.show()
    ...
    def calculate_td_cyclotron(
        ...
        if second_peak_inx.shape[0] < 1:
            ...
            second_peak_delay = self.delays[
                td_min_bin + first_peak_inx + 1 + second_peak_inx
            ]

            fratio = second_peak_delay / first_peak_delay
            if fratio > 1.6667:
                err = second_peak_delay - 2.0 * first_peak_delay
                # print 'No correction, 2.0 = %f' % fratio
            elif (fratio <= 1.6667) & (fratio > 1.4):
                err = second_peak_delay - 3.0 / 2.0 * first_peak_delay
                first_peak_delay = first_peak_delay / 2.0
                # print 'Correction 1.5 ~= %f [2nd harmonic]' % fratio
            elif (fratio <= 1.4) & (fratio > 1.2857):
                err = second_peak_delay - 4.0 / 3.0 * first_peak_delay
                first_peak_delay = first_peak_delay / 3.0
        ...
        if (first_peak_delay < ais_td_cyclotron_sensible[0]) | (
            ...
            self.digitization.set_cyclotron(
                first_peak_delay,
                np.abs(err),
                method="AUTO - V" + __auto_version__,
            )
        ...
        if figure_number:
            ...
            plt.vlines(first_peak_delay * 1.0e3, *plt.ylim(), colors="blue")
            plt.vlines(
                second_peak_delay * 1.0e3,
                *plt.ylim(),
                colors="blue",
                linestyle="dashed",
            )
            plt.vlines(
                np.arange(0.0, plt.xlim()[1], first_peak_delay * 1.0e3),
                *plt.ylim(),
                colors="red",
                linestyle="dotted",
            )
    ...
    def calculate_reflection(
        self, frequency_range=None, h=None, delay_range=(1e-3, 7e-3)
    ):
        d = self.digitization
        d.delete_trace()
        ...
        delay = 2.0 * (pos[0] - h) / speed_of_light_kms
        ...
        d.set_trace(
            np.array(
                (
                    2.0 * (pos[0] - 200.0) / speed_of_light_kms,
                    2.0 * (pos[0] - 25.0) / speed_of_light_kms,
                )
            ),
            np.array(frequency_range) * 1e6,
            method="AUTO-INTERMEDIATE",
        ...
        if d.traced_delay.size:
            ...
            #               np.mean(np.diff(d.traced_delay)),
    ...
    def refine_trace(
        ...
        """Automagically adjust the current trace to fit peaks in the data,
        above *threshold*. max_delay is numeric to specify some maximum, or
        'ground' for the determined ground, or else
        """
        ...
        for i, s in enumerate(other):
            ...
            distance = (self.delays[d] - new_delays[i]) / (2.0 * width)
        ...
        # Retain only after last absolute change of more than 3 delay incrs
    ...
    def __str__(self):
        ...
        ss = [
            ...
            if (k not in DIGITIZATION_BANNED_KEYS) and (k[0] != "_")
        ]
        return s + "\n".join(ss)
...
class AISTimeSeries(object):
    ...
    def plot_spectrum(self, ax=None, time_range=None, vmin=None, vmax=None):
        ...
        for ionogram in self.ionogram_list:
            ...
            plt.scatter(
                np.zeros_like(data) + ionogram.time,
                ionogram.frequencies / 1.0e6,
                c=data,
                marker="s",
                hold=True,
                vmin=-16,
...
class _AISEmpiricalCalibrationOLD(object):
    ...
    def __init__(self, fname=None, orbits=None):
        ...
        if fname is None:
            fname = irfuplanets.config["mex"]["data_directory"] + "ais_calfile"
    ...
    def calibrate(self, points=10):
        ...
        if not hasattr(self, "t"):
            self._construct_calibration_array()
    ...
    def plot(self):
        ...
        for i in range(self.t.shape[0]):
            # if i % 10 != 0: continue
            val = 10.0 ** np.interp(self.x[i], self.cal[0], self.cal[1])
            err = 10.0 ** np.interp(self.x[i], self.cal[0], self.cal[2])
        ...
        plt.figure()
        plt.hist(np.abs(dists), bins=20, range=(0.0, 4.0))

        # plt.figure()
        # plt.hist(sigmas, bins=20)
        dists = np.abs(dists)
        # some statistics:
        s = 100.0 / float(self.t.shape[0])
        print()
    ...
    def __call__(self, ig):
        ...
        return 10.0**val, 10.0**val * err / 0.434
...
class AISEmpiricalCalibration(object):
    ...
    def previous_previous(self, ig):
        ...
        # issue is with fitting the large variation meaningfully -
        # lots of outliers lead to wild distortions
        # This fit (more or less) maximises the peaked-ness of the resulting
...
class IonogramDigitization:
    ...
    def _compute_fp_local(self, threshold=109982.0):
        ...
        # both need to be finite to compare,  if integrated is nan, density
        # is low therefore ignore morphology as well
        ...
        if self.fp_local_manual_lines is not None:
            if self.fp_local_manual_lines.size > 1:
                ...
                self.fp_local_error = np.mean(
                    np.abs(
                        self.fp_local_manual_lines
                        - np.arange(1, self.fp_local_manual_lines.size + 1)
                        * self.fp_local
                    )
                )
            if self.fp_local_manual_lines.size == 1:
    ...
    def set_morphology_fp_local(
        self, fp_local, fp_local_error, method, selected_f=None, length=None
    ):
    ...
    def __eq__(self, other):
        ...
        return abs(self.time - other.time) < (ais_spacing_seconds / 2.0)
    ...
    def __bool__(self):
        if np.isfinite(self.fp_local):
            return True
        if np.isfinite(self.td_cyclotron):
            return True
        if np.isfinite(self.morphology_fp_local):
            return True
        if np.isfinite(self.integrated_fp_local):
            return True
        if np.isfinite(self.ground):
            return True

        if self.traced_delay.size > 0:
            return True
    ...
    def is_invertible(self, strict=False):
        ...
        if self.traced_delay.size < 4:
            return False
        if self.traced_delay.shape != self.traced_frequency.shape:
            return False
        return True
...
class DigitizationDB:
    """Class that keeps track of the digitizations, only allowing one instance
    per ais frame."""
    ...
    def add_single(self, digitization):
        if "IonogramDigitization" in str(type(digitization)):
            if digitization not in self._digitization_list:
...
def compute_all_digitizations(orbit, filename=None, verbose=False):
    ...
    for e, i in enumerate(ionogram_list):
        ...
        if "failed" not in i.calculate_fp_local().lower():
            fp_local_counter += 1
        if "failed" not in i.calculate_ground_trace().lower():
            ground_counter += 1
        if "failed" not in i.calculate_reflection().lower():
            ion_counter += 1
        if "failed" not in i.calculate_td_cyclotron().lower():
            td_cyclotron_counter += 1
...
def produce_ne_b_file(
    ...
    if "$DATE" in file_name:
        file_name = file_name.replace("$DATE", utcstr(now(), "ISOC")[:10])

    f = open(file_name, "w")
    f.write(
        "# <UTC Time> <Ne / cm^-3> <Ne Error / cm^-3> <B / nT> <B Error / nT> "
        "<Ne_max / cm^-3> \n"
    )
    f.write(
        """# Note: For Ne < 600 or B > 70, these values should be considered "
        "to be upper and lower limits, respectively\n"""
    )
    no_records = 0
    orb_records = 0
    last_orbit = 0
    ...
    for o in orbits:
        ...
        for d in all_digs:
            ...
            if hasattr(d, "td_cyclotron"):
                ...
                b_err = np.abs(b_err)
                # b_err = b - td_to_modb(
                #       d.td_cyclotron + d.td_cyclotron_error) * 1.E9
            ...
            if write:
                ...
                f.write(
                    "%s %f %f %f %f %f\n"
                    % (
                        spiceet_to_utcstr(d.time, precision=0),
                        ne,
                        ne_err,
                        b,
                        b_err,
...
def write_yearly_ne_b_files(years=None, directory="."):
    if years is None:
        years = list(range(2005, int(utcstr(now())[:4]) + 1))

    for y in years:
        start = mex.orbits[spiceet("%04d-001T00:00" % y)].number
        finish = mex.orbits[spiceet("%04d-001T00:00" % (y + 1))].number - 1
...
def _generate_ais_coverage(start=1, finish=None, outfile=None):
    ...
    if finish is None:
        # finish = mex.orbits[now()- 86400. * 365].number
...
def plot_ais_coverage_bar(
    ...
    p = ax.get_position()
    new_ax = fig.add_axes(
        [p.x0, p.y1, p.x1 - p.x0, height], xticks=[], yticks=[], sharex=sharex
    )
    ...
    x = np.array([1.0, 0.0, 0.0, 1.0, 1.0])
...
def _generate_ais_index(
    ...
    field_model = CainMarsFieldModel()
    out = {}
    if update:
    ...
    if finish_orbit is None:
        finish_orbit = mex.orbits[now()].number - 50
    count = 0
    ...
    for o in range(start_orbit, finish_orbit):
        ...
        a["mso_rll"], a["mso_pos"], a["sza"] = mex.mso_r_lat_lon_position(
    ...
    print("Finished")
    print("Wrote %d records to %s" % (count, outfile))
    print("%d calculation errors" % (calculation_error_count))
...
def get_ais_data(
    ...
    model_inclination=None,
    density_range=None,
    description=False,
):
    """Returns AIS densities that satisfy criteria. Longitudinal criteria are
    interpreted as being (L0, DL) to give L0 - DL to L0 + DL
    (around the circle)"""
    ...
    data = mex.datastore["ais_index"]

    if not time:
        time = (FREQUENCY_TABLE_FIXED_AFTER, now())
    ...
    alt -= constants.mars_mean_radius_km
    ...
    def calc_incl(x):
        return np.arctan2(
            x["b_model"][0],
            np.sqrt(x["b_model"][1] ** 2.0 + x["b_model"][2] ** 2.0),
        )
    ...
    if sub_solar_longitude:
        ...
        diff = np.abs(
            angle_difference(sub_sol_lon, sub_solar_longitude[0], degrees=True)
        )
    ...
    if longitude:
        diff = np.abs(angle_difference(lon, longitude[0], degrees=True))
...
if __name__ == "__main__":
    ...
    #          d.set_trace(np.random.randn(10),
    #                       np.random.randn(10), method='X')
    #          try:
    #              a.add(d)
