...
class AISTool(object):
    ...
    def __init__(
        ...
        self._histogram_figure_number = None

        self.auto = auto

        self.ionospheric_model = Morgan2008ChapmanLayer()

        np.seterr(all="ignore")

        self.params = dict(auto_refine=False, substitute_fp=ais.ne_to_fp(4.0))
        ...
        g = mpl.gridspec.GridSpec(
            ...
            2,
            width_ratios=[1, 0.34],
            height_ratios=[0.001, 0.001, 7, 5, 2, 16],
            wspace=0.16,
            hspace=0.1,
            left=0.05,
            right=0.95,
    ...
    def set_orbit(self, orbit, strict=True):
        ...
        while not successfully:
            ...
            if strict or attempts > 10:
                raise mex.MEXException(
                    "Orbit not found - no data, missing file, or some other"
                    " bollocks."
                )
        ...
        # If the user specified one, load it,
        ...
        if self.debug:
            ...
            print(
                "Expected number of ionograms = %d, found = %d"
        ...
        # Trajectory info
        self.trajectory = {}
        self.trajectory["t"] = np.arange(self.extent[0], self.extent[1], 60.0)
        pos = mex.iau_mars_position(self.trajectory["t"])
        self.trajectory["pos"] = pos / constants.mars_mean_radius_km
            ...
            for i, ig in enumerate(self.ionogram_list):
                if ig is self.current_ionogram:
                    if ((i + ig_inc) > 0) and (
                        ...
                        self.set_orbit(
                            self.orbit + int(ig_inc / abs(ig_inc)),
                            strict=False,
                        )
        ...
        if event.inaxes == self.ig_ax:
            if self.status is None:
                ...
                if len(self.selected_plasma_lines) >= 1:
                    ...
                    self.message(
                        "Selected Plas. Line. @ %f MHz, Morph Fp_loc = %f MHz"
                        % (
                            event.xdata,
                            self.current_ionogram.digitization.fp_local / 1e6,
                        )
                    )
                    self.update()
        ...
        if event.inaxes == self.ig_ax:
            ...
            if self.status == "go_tracing":
                ...
                if len(self.traced_delays) > 2:
                    plt.plot(
                        self.traced_frequencies[-2:],
                        self.traced_delays[-2:],
    ...
    def on_release(self, event):
        ...
        if event.inaxes == self.ig_ax:
            ...
            if self.status == "go_tracing":
                ...
                d.set_trace(
                    np.array(self.traced_delays) * 1.0e-3,
                    np.array(self.traced_frequencies) * 1.0e6,
                    method="MANUAL",
                )
                ...
                self.message(
                    ...
                    if d.invert(substitute_fp=self.params["substitute_fp"])
                    else "Inversion failed"
                )
                self.update()
            ...
            #         print 'TD_CYCLOTRON = ', d.td_cyclotron,
            #                    d.td_cyclotron_error
    ...
    def on_scroll(self, event):
        ...
        if self.status == "plasma_lines":
            ...
            if not np.isfinite(fp):
                ...
                new_fp = (event.step * 0.005 + 1.0) * fp
    ...
    def auto_fit(
        self,
        plasma_lines=True,
        cyclotron_lines=True,
        ionosphere=True,
        ground=True,
        new_digitization=False,
        update=True,
    ):
    ...
    def update(self):
        ...
        # does nothing if not required:
        self.current_ionogram.interpolate_frequencies()
        ...
        freq_extent = (
            self.extent[0],
            self.extent[1],
            ais.ais_max_delay * 1e3,
            ais.ais_min_delay * 1e3,
        )
        inx = (
            1.0e6
            * (
        ...
        title = "AISTool v%s, Orbit = %d, Ionogram=%s " % (
            __version__,
            self.orbit,
        ...
        if self.minimum_interaction_mode:
            title += "[Quick] "
        if self._digitization_saved is False:
            title += "UNSAVED "
        if self.get_status() is not None:
            title += "[Status = %s] " % self.get_status()

        pos, sza = mex.mso_r_lat_lon_position(
            float(self.current_ionogram.time), sza=True
        )

        title += (
            "\nMSO: Altitude = %.1f km, Elevation = %.1f,"
            " Azimuth = %.1f deg, SZA = %.1f"
            % (
        ...
        loc_f = irftime.SpiceetLocator()
        loc_t = irftime.SpiceetLocator()
        self.freq_ax.xaxis.set_major_formatter(irftime.SpiceetFormatter(loc_f))
    ...
    def digitization_saved(self):
        ...
        #     # print 'len 0'
        #     if self.debug:
        #       print "Shouldn't have a 0 length list of digitizations"
        #     return False
        if self.current_ionogram.digitization not in self.digitization_db:
            # print 'is empty?'
    ...
    def on_keypress(self, event):
        ...
        a: auto fit trace, plasma, cyclotron, ground position
            * plasma and cyclotron lines are implemented,
            but ground and trace aren't yet
        w: manual plasma freq (click adjacent lines, w again to stop)
        c: manual cyclotron freq (click adjacent lines, c again to stop)
        t: manual trace ionogram (draw near trace, release, peak
                fitting is then done)
        e: edit trace #
        ...
        z,x: show previous, next plasma, cyclotron and traces
             on top of current#
        ...
        if event.key:
            ...
            if self.status != "orbit":
                ...
                if f:
                    if self.minimum_interaction_mode and (
                        event.key not in ["enter", "n", "right"]
                    ):
                        self.minimum_interaction_mode = False
                        self.set_status(None)
                    f()
                ...
                if event.key == "n":
                    if self.debug:
                        print("Loading next orbit")
                    self.set_orbit(self.orbit + 1, strict=False)
                    self.set_status(None)
                    return
                if event.key == "p":
                    if self.debug:
                        print("Loading previous orbit")
                    self.set_orbit(self.orbit - 1, strict=False)
                    self.set_status(None)
                    return
                if event.key in [
                    ...
                    print(
                        "Didn't recognise the orbit number (%s) - n "
                        "= next, p = previous, otherwise 5 digits"
                        % ("".join(self.new_orbit))
                    )
                    self.set_status(None)
    ...
    def key_q(self):
        ...
        print("-----")
        print(
            "Totals: FP = %d, TD = %d, ground = %d, reflection = %d"
            % (fp, td, ground, reflection)
        )
        self.digitization_db.write()
    ...
    def key_h(self):
        ...
        plt.hist(
            np.log10(self.current_ionogram.data.flatten()),
            bins=np.arange(-25.0, -9.0, 0.1) - 0.05,
            fc="none",
        )
