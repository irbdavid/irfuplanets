...
def load_kernels(
    ...
    if load_all:
        ...
        start = spiceet("2003-06-01T00:00")
        finish = spiceet(now() + 10.0 * 86400.0 * 365.0)
    ...
    if time is None:
        ...
        if hasattr(time, "__len__"):
            ...
            start = time
            finish = time
        start_str = utcstr(start, "ISOC")
        finish_str = utcstr(finish, "ISOC")
    ...
    if "NONE" not in last_spice_time_window:
...
def unload_kernels():
    ...
    try:
        ...
        # a) irfuplanets was loaded first (safe assertion, this code won't work
...
def recpgr(pos, body="MARS"):
    ...
    if body == "MARS":
        ...
        raise NotImplementedError("Unknown body: " + body)
...
def iau_r_lat_lon_position(time, **kwargs):
    """Return the position of MEX at `time`, in Radial Distance/Latitude/EAST
    Longitude. No accounting for the oblate spheroid is done,
    hence returning radial distance [km]
    """
...
# Convert "WEST" longitudes to "EAST", which seems to be more commonly used.
# This is the reason for the -1 * multiplication of the longitude here,
# and the *1 in the corresponding iau_pgr_alt_lat_lon_position function
def iau_pgr_alt_lat_lon_position(time, **kwargs):
    """Return the position of MEX at `time`, in
    ...
    out[2] = 360.0 - 1.0 * np.rad2deg(
        tmp[0]
    )  # Convert to EAST LONGITUDE, RECPGR returns in [0, 2pi], so add 360.
    return out
...
def solar_longitude(t, body="MARS", correction="NONE"):
    """Calculate solar longitude, default Mars.

    Nb. PlanetoCENTRIC coordinates, and this is NOT a synonym for
    sub-solar longitude (PlanetoGRAPHIC longitude).  Defined as
        Ls = solar_longitude = 0 deg at NH vernal equinox,
        Ls = 90 deg a NH summer solstice etc."""
...
def sub_solar_longitude(et):
    """Return the sub-solar longitude on MARS at time `et`,
    in EAST longitude (degrees)."""
    ...
    return np.rad2deg(2.0 * np.pi - tmp[0])
    ...
    with open(fname) as f:
        ...
        for line in f.readlines():
            try:
                ...
                tmp = Orbit(
                    n,
                    utcstr_to_spiceet(parts[4]),
                    utcstr_to_spiceet(parts[1]),
                    name="MEX",
                )
...
def read_all_mex_orbits(recompute=False, allow_write=True, verbose=False):
    ...
    if verbose:
        print(
            "Predicted orbits read     : %d - %d (%d)"
            % (min(predicted.keys()), max(predicted.keys()), len(predicted))
        )
        print(
            "Reconstructed orbits read: %d - %d (%d)"
            % (min(real.keys()), max(real.keys()), len(real))
        )
    ...
    if verbose:
        print(
            "Inserted %d predicted values (min = %d):"
            % (len(overwritten), min(overwritten))
        )
...
def plot_mex_orbits_bar(
    ...
    # new_ax.xaxis.set_major_locator(
