...
class IonosphericModel(object):
    ...
    At the minimum, __call__ must be overridden"""
    ...
    def get_tec(self, theta=0.0, hmax=np.inf):
        raise NotImplementedError("Derived must override")
    ...
    def ais_response(
        ...
        point. This is done empirically, rather than analytically such that
        models can be more or less arbitrarily defined"""

        # The inversion must be done again, in reverse to produce
        # the 'dispersed' delays?
...
class ChapmanLayer(IonosphericModel):
    ...
    def fit(self, altitude, density, theta):
        ...
        Takes the peak of the layer to be the peak density, rather than
        fitting it in concert with the other parameters - see fitx() for this
        """

        if np.any(np.diff(altitude) < 0.0):
            raise ValueError(
                "fit() requires altitudes sorted increasing, and corresponding"
                " densitities"
            )
        ...
        self.n0 = density[i] / np.exp((1.0 - 1.0 / np.cos(theta)) / 2.0)
        ...
        h, success = opt.leastsq(_err, 10.0)
        self.h = h
        self.fit_success = success
        return success

    def fitx(self, altitude, density, theta):
        """Fit this model to some profile.  Requires values sorted by
        altitude. c.f. fit()"""
        if np.any(np.diff(altitude) < 0.0):
            raise ValueError(
                "fit() requires altitudes sorted increasing, "
                "and corresponding densitities"
            )
        ...
        x0 = [density[i], altitude[i], 10.0]

        p, success = opt.leastsq(_err, x0)
...
class FunctionalModel(IonosphericModel):
    """A model that is defined only by a function that returns density at
    altitude, sza"""
...
class TabulatedModel(IonosphericModel):
    ...
    def __init__(self, altitude, density, theta=0.0):
...
class CompositeModel(IonosphericModel):
    def __init__(
        ...
        base=None,
        local_density=0.0,
        local_scale_height=50.0,
        bottomside_density=None,
    ):
        if base is None:
    ...
    def __call__(self, alt, theta=0.0):
        log_bit = self.local_density * np.exp(
            -(alt - np.amax(alt)) / self.local_scale_height
        )
        ...
        if models:
            for m in models:
                if not isinstance(m, IonosphericModel):
                    raise ValueError(
                        "WHAT THAR FUCK? Not an IonosphericModel instance!"
                    )
...
def plot(all_models):
    ...
    for m in all_models:
        ...
        plt.plot(freq / 1e6, delay * 1e3, lw=2.0)

    plt.hlines(
        -2 * np.amax(alt) / speed_of_light_kms * 1e3,
        *plt.xlim(),
        linestyle="dashed",
    )
...
def _laminated_delays_test(dd, f, fp_local, altitude=None):
    ...
    for i in range(1, nf):
        freq_rat[i, i] = np.pi / 2.0

    cos_f = np.cos(freq_rat)
    exp_w = 0.5 * np.log((1.0 - cos_f) / (1.0 + cos_f))
    ...
    for i in range(2, nf):
        alpha[i] = -diff_exp[i, i] / (
            app_range[i] + np.sum(diff_exp[i, 1:i] / alpha[1:i])
        )
...
if __name__ == "__main__":
        ...
        all_models.append(
            ChainedModels(
                (
                    ...
                    GaussianModel(12e4, 220.0, 70.0),
                    GaussianModel(n / 5, 200, 10),
                )
            )
        )
    ...
    f = 10.0 ** (np.linspace(np.log10(150e3), np.log10(5.5e6), 160))

    ax = plt.subplot(211)
    h = np.linspace(alt, 0.0, 160)

    n = model(h)
    plt.plot(n, h, "kx-")
    plt.ylim(0.0, 1000.0)
    plt.xscale("log")
    t, f = model.ais_response(
        frequencies=f, sc_altitude=alt, altitude_resolution=alt_res
    )
    t = -1.0 * t
    ...
    plt.sca(ax)
    plt.plot(nn2, hh2 / 1000.0, "r+-")
    plt.plot(nn, hh / 1000.0, "b*-")
    plt.xlim(1.0, 1e6)
    plt.show()
