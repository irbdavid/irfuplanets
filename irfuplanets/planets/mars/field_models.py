...
from irfuplanets.data import polar_to_cartesian
from irfuplanets.planets.mars import constants
from irfuplanets.plot import CircularLocator, make_colorbar_cax
from irfuplanets.time import SpiceetFormatter, SpiceetLocator, spiceet, utcstr
...
def schmidt_polynomials(x, nmax):
    """Returns an *nmax*+1 by *nmax*+1 matrix containing Schmidt
    quasi-normalised polynomials evaluated at *x*.  Written with and tested
    against a bit of code from D. Brain.
    """
    ...
    for m in range(1, nmax + 1):
        ...
        arr[m, m] = (1.0 - x**2.0) ** (0.5 * m) * cm

        for i in range(1, m):
            arr[m, m] = (2.0 * i + 1.0) * arr[m, m]

        if nmax > m:
            twoago = 0.0
            for i in range(m + 1, nmax + 1):
                arr[i, m] = (
                    x * (2.0 * i - 1.0) * arr[i - 1, m]
                    - np.sqrt((i + m - 1.0) * (i - m - 1.0)) * twoago
                ) / np.sqrt(i**2 - m**2)
                twoago = arr[i - 1, m]
    return arr
...
def convert_biau_to_bmso(iau_position_rll, b_iau, times, check_kernels=None):
    ...
    Result is in the MSO frame, cartesian components.

    Will by default assume that the correct spice kernels are loaded
    (since you have presumably just calculated iau_position_rll).  Otherwise,
    set check_kernels=True, which defaults to using MEX routines, or the
    supplied package, e.g.
        import maven;
...
class SphericalHarmonicModel(FieldModel):
    ...
    def __init__(
        ...
        # print """SphericalHarmonicModel: Still not sure about the absolute
        #  values calculated..."""
    ...
    def pure_python_call(self, pos, nmax=None):
        """Evaluate this model at position *pos* where pos is a 3xN array
        containing position coordinates in r (km), lat (deg), lon (deg)"""
        if nmax is None:
            nmax = self.g.shape[0] - 1
    ...
    def _eval(self, p, nmax):
        """*p* is position in [r, lat, lon], where r is in km,
        and lat, lon are in deg"""
        # This is the bit that needs to be written in C
        # This routine is shamelessly stolen from Dave Brain, and transposed
        # to python.  Thanks Dave!, Cheers, /OtherDave.
        a_over_r = self.rp / p[0]
        l = np.deg2rad(p[2])  # noqa E741
        sct = np.deg2rad(90.0 - p[1])
        ...
        # ;  Theta(theta) = P(n,m,x)  the Schmidt normalized associated
        # ;     legendre poly.
        ...
        bp = -1.0 * np.sum(bp * r) * a_over_r / sin_theta
...
class ArkaniMarsFieldModel(SphericalHarmonicModel):
    ...
    def __init__(self, coefficients_file=None, nmax=62):
        ...
        if (self.nmax + 2) > tmp.ga.shape[0]:
            raise ValueError(
                "nmax = %d greater than supplied g,h shape (%d,%d)"
                % (nmax, tmp.ga.shape[0], tmp.ga.shape[0])
            )
...
class CainMarsFieldModel(SphericalHarmonicModel):
    ...
    def __init__(self, coefficients_file=None, nmax=60):
        ...
        if coefficients_file is None:
            # coefficients_file = irfuplanets.config["mex"]["data_directory"] +
            # 'cain03_coefficients.dat'
            coefficients_file = (
                irfuplanets.config["mex"]["data_directory"]
                + "brain/martiancrustmodels.sav"
            )
...
class LillisMarsFieldModel(SphericalHarmonicModel):
    ...
    def __init__(self, coefficients_file=None, nmax=51):
        ...
        with open(coefficients_file) as f:
            ...
            for line in f:
                c += 1
                n, m, gnm, hnm = line.split()
                n = int(n)
                m = int(m)
                self.g[n, m] = float(gnm)
                self.h[n, m] = float(hnm)

        print("Read %d lines of coefficients from %s" % (c, coefficients_file))
...
#                 raise ValueError(
#                     "%d orbits, but not many points requested? "
#                     " force=True disables this check"
#                 )
...
#             try:
#                 chunk = np.load(
#                     self.directory + "%05d/%05d.npy" % (o / 1000 * 1000, o)
#                 )
#             except IOError:
#                 compute_count += 1
#                 print("Computing for orbit %d" % o)
#                 o_t = (
#                     np.arange(-40.0 * 60.0, 40.0 * 60.0, 5.0)
#                     + mex.orbits[o].periapsis
#                 )
...
#         for i in (0, 1, 2):
#             output[i, :] = np.interp(
#                 t, data[0, :], data[i + 1, :], left=np.nan, right=np.nan
#             )
...
def create_snapshot(model, filename=None, resolution=1.0, altitude=150.0):
    ...
    obj = dict(
        latitude=latitude,
        longitude=longitude,
        latitude_mesh=lat_mesh,
        longitude_mesh=lon_mesh,
        altitude=altitude,
        model=model.name,
        field=field,
    )
...
def plot_lat_lon_field(
    ...
    inclination_draped_imf=0.0,
    inclination_draped_imf_orient=0.0,
    snapshot_kwargs={},
    data_only=False,
    **kwargs,
):
    ...
    if name is None:
        name = (
            irfuplanets.config["mex"]["data_directory"]
            + "saved_field_models/Cain_400km1deg.pck"
        )
        if not os.path.exists(name):
            name = (
                irfuplanets.config["mex"]["data_directory"]
                + "saved_field_models/CainMarsFieldModel_1deg_400km.pck"
            )
    ...
    q_abs = False
    units = "nT"
    _vmin, _vmax = 0.0, 50.0

    if field_data["altitude"] < 200.0:
        _vmax = 500.0
    ...
    if value == "|B|":
        q = np.sqrt(
            (fd[0, :].reshape(l_mesh_shp).T) ** 2.0
            + (fd[1, :].reshape(l_mesh_shp).T) ** 2.0
            + (fd[2, :].reshape(l_mesh_shp).T) ** 2.0
        )
        q_abs = True
        ...
        fd[1, :] += inclination_draped_imf * np.cos(
            np.deg2rad(inclination_draped_imf_orient)
        )
        fd[2, :] += inclination_draped_imf * np.sin(
            np.deg2rad(inclination_draped_imf_orient)
        )

        q = np.rad2deg(
            np.arctan2(
                fd[0, :].reshape(l_mesh_shp).T,
                np.sqrt(
                    (fd[1, :].reshape(l_mesh_shp).T) ** 2.0
                    + (fd[2, :].reshape(l_mesh_shp).T) ** 2.0
                ),
    ...
    return out
    # if irfuplanets.time.now() > spiceet("2012 JULY 11"):
    #     raise NotImplementedError("Unfinished / unchecked 2012-07-08")


if __name__ == "__main__":
    # So the conclusion seems to be that the above code reproduces the field
    # geometry well, except insofar as the bigger fields seem a bit damped
    # compared to the duru paper. These are due to the higher order terms,
    # which only dominate close to in, so maybe its a precision thing, but then
    # all the above is done in 64bit, so I doubt that the error is here if
    # that's the issue. Doesn't also seem to be particularly sensitive to
    # planetographic coordinates or not (i.e. whether we account for the
    # oblate spheriod, unless again this isn't being done correctly)
    ...
    if True:
        ...
        for alt in (400.0, 0.0, 100.0, 150.0, 1000.0):
            if alt < 10.0:
                alt = 10.0
            fname = name % alt
    ...
    if False:
        ...
        print(a.rp)
        print(constants.mars_mean_radius_km)
        # r = np.zeros(100) + 1.001 * constants.mars_mean_radius_km
        ...
        if comparison == "Duru":
            et = np.linspace(
                spiceet("2005-224T04:45:00"),
                spiceet("2005-224T05:15:00"),
                100,
            )
        ...
        if comparison == "Fraenz":
            orb = mex.orbits[5009]
            et = np.linspace(-7 * 60.0, 7 * 60.0, 100) + orb.periapsis

        pos = mex.iau_r_lat_lon_position(et)
        # pos[0,:] += constants.mars_mean_radius_km
        # pos = np.empty_like(p)
        ...
        plt.subplot(311)
        plt.plot(et, pos[0, :] - constants.mars_mean_radius_km)
        # plt.plot(et, p[0,:] - constants.mars_mean_radius_km)
        ...
        print(utcstr(et[0]))
        print(utcstr(et[-1]))
        plt.gca().xaxis.set_major_locator(SpiceetLocator())
        plt.gca().xaxis.set_major_formatter(SpiceetFormatter())
    ...
    if False:
        ...
        plt.imshow(
            theta.reshape(lat.shape).T,
            vmin=0.0,
            vmax=90.0,
            extent=(0.0, 360.0, -90.0, 90),
        )
        plt.xlabel("Longitude / deg")
        plt.ylabel("Latitude / deg")
        plt.colorbar()
