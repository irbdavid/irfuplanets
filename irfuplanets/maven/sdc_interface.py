...
from irfuplanets.time import (  # Orbit,; OrbitDict,; now,; setup_time_axis,
    spiceet,
    utcstr,
)
...
def yyyymmdd_to_spiceet(x):
    return spiceet(x[:4] + "-" + x[4:6] + "-" + x[6:8] + "T00:00")
...
class HTTP_Manager(object):
    ...
    def __init__(
        ...
        Args:
            ...
            update_interval: time in seconds before an index.html file is
    ...
    def query(
        ...
        Will first query the remote server, download missing files, and then
        delete local files that match the query but are no longer present on
        the remote. The implicit assumption here is that the remote directory
        is PERFECTLY maintained.

        Args:
            query: query string with wildcards to locate a file
                on the remote server, e.g.
                'sci/lpw/l2/2015/01/mvn_lpw_l2_lpnt_*_v*_r*.cdf'

            version_function: takes the expanded wildcards from the query,
                and converts them to a number used to compare versions and
                releases (higher=better). For example:
                    lambda x: (x[0], float(x[1]) + float(x[2])/100.)
                to generate 1.02 for V1, R2 for the above query (2nd and
                3rd wildcards)

            date_function: takes the expanded wildcards from the query, and
                converts to a date for the content of the file, for example:
                    lambda x: yyyymmdd_to_spiceet(x[0])
                for the above query example.

            start: start date SPICEET, ignored if not set
            finish: finish date, ignored if not set. 'finish' must be set if
                'start' is (can use np.inf, if you want)

        Returns: List of local files, freshly downloaded if necessary,
            that satisfy the query supplied.
        """
        ...
        def _version_function(x):
            return 0, "".join(x)

        if version_function is None:
            version_function = _version_function

        self.current_re = re.compile(query_filename.replace("*", r"(\w*)"))
        ...
        if check_time:
            ...
            finish_day = (
                spiceet(utcstr(finish, "ISOC")[:10]) + 86398.0
            )  # 1 day - 1s - 1 (possible) leap second

        # if verbose:
        #   print 'Remote path: ', self.remote_path + query_base_path

        # key will be the unique id of the file,
        # value will be (version, the full name, local == True)
        ok_files = {}
        files_to_delete = []
            ...
            if tmp:
                ...
                if unique_id in ok_files:
                    if ok_files[unique_id][0] < version_number:
                        ok_files[unique_id] = (
                            version_number,
                            self.local_path + query_base_path + f,
                            True,
                        )
                else:
                    ok_files[unique_id] = (
        ...
        if verbose:
            if ok_files:
                print(
                    "%d local matches with highest version %f"
                    % (
                        len(ok_files),
                        max([v[0] for v in list(ok_files.values())]),
                    )
                )
            else:
        ...
        if self.download:
            ...
            if update_index:
                try:
                    ...
                    if ok_files:
                        msg = """No remote index available, but local matches
                            were found anyway. This should never happen.
                            Details:
                            remote_path: %s
                            index_path: %s
                            """
                        raise RuntimeError(msg.format(remote_path, index_path))

                    return []

            with open(index_path) as f:
                remote_files = self.index_parser.extract_links(
                    f.read()
                )  # without the remote + base path

            if not remote_files:
                raise IOError("No remote files found from index file")

            # inspect each file, remove if it doesn't match the query,
            # or is not the most recent version
            for f in remote_files:
                ...
                if tmp:
                    ...
                    if unique_id in ok_files:
                        if ok_files[unique_id][0] < version_number:
                            # if we are overwriting a local entry, we will also
            ...
            if verbose:
                if ok_files:
                    print(
                        "%d remote matches with highest version %f"
                        % (
                            len(ok_files),
                            max([v[0] for v in list(ok_files.values())]),
                        )
        ...
        if not silent:
            print(
                "Query %s: Returning %d (DL: %d, DEL: %d)"
