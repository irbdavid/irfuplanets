...
# INSERTION_DATE = spiceet("2014-09-22T02:24")
# LAUNCH_DATE = spiceet("2013-11-17T18:28")
...
def load_kernels(
    ...
    if load_all:
        ...
        start = spiceet("2013-11-19T00:00")
    ...
    try:
        kernel_directory = irfuplanets.config["maven"]["kernel_directory"]
        if verbose:
        ...
        if start_int > -999999:
            ...
            for f in iglob(kernel_directory + "spk/maven_orb_rec_*.bsp"):
                ...
                if this_finish < start_int:
                    continue
                if this_start > finish_int:
                    continue
                spiceypy.furnsh(f)
                load_count += 1
                if verbose:
                ...
                if this_start > max_encountered:
                    max_encountered = this_start
                if this_finish > max_encountered:
                    max_encountered = this_finish
...
def iau_r_lat_lon_position(time, **kwargs):
    """ " Return the position of MEX at `time`, in
            Radial Distance/Latitude/EAST Longitude.
    No accounting for the oblate spheroid is done, hence returning
    radial distance [km]
    """
...
# Convert "WEST" longitudes to "EAST", which seems to be more commonly used.
# This is the reason for the -1 * multiplication of the longitude here, and
# the *1 in the corresponding iau_pgr_alt_lat_lon_position function
...
def sub_solar_longitude(et):
    ...
    def f(t):
        pos, _lt = spiceypy.spkpos("SUN", t, "IAU_MARS", "NONE", "MARS")
        return np.array(pos)
...
def read_maven_orbits(fname, verbose=False):
    ...
    # Lockheed putting in the periapsis of the orbit, and the terminating
    # apoapsis, i.e. wrong way round
...
if __name__ == "__main__":
    ...
    if True:
        ...
        t0 = spiceet("2013-001T00:00")
        t1 = spiceet("2016-001T00:00")
        for t in np.arange(t0, t1, 86400.0):
            try:
                pos = position(t)
                plt.plot(
                    t,
                    np.sqrt(pos[0] ** 2.0 + pos[1] ** 2.0 + pos[2] ** 2.0),
                    "ko",
                )
