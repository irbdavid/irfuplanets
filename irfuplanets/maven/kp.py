...
class MavenKPData(dict):
    """A helper class that organises the KP data dictionary, allowing
    getting/setting via attributes as well as dictionary keys."""
...
#     This relies on having the columns in the file synced with those
#       in the data.  This seems to be the case. """
#
#     output = {}
...
def kp_read_files(files, verbose=False):
    ...
    'short' names for the various data products are generated on the fly,
    and will not match those used in the IDL toolkit.  The neccessary
    information to construct an exact copy is not contained in the KP files."""
    ...
    converters = {}
    converters[0] = spiceet  # Conver the ISO UTC string to a SPICEET
...
if __name__ == "__main__":
    ...
    t0 = spiceet("2015-05-01")
    t1 = t0 + 86400.0 * 10.0 - 1
    # t1 = t0 + 86400. - 1.
